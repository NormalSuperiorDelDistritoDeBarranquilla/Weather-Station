    ...
    if value >= 42:
        alerts.append(
            build_alert(
                ...
                recommendation="Verificar condiciones del entorno y proteger instrumentación expuesta.",
                current_value=value,
        ...
        alerts.append(
            build_alert(
                ...
                recommendation="Monitorear el aumento y revisar ventilación, sombreado o exposición directa.",
                current_value=value,
                unit=METRIC_REGISTRY["temperature"].unit,
    ...
    if value <= -2.2:
        alerts.append(
            build_alert(
                ...
                recommendation="Proteger instrumentación y revisar impacto sobre superficies expuestas.",
                current_value=value,
                unit=METRIC_REGISTRY["temperature"].unit,
        ...
        alerts.append(
            build_alert(
                ...
                recommendation="Activar medidas de protección para equipo sensible a congelación.",
                current_value=value,
                unit=METRIC_REGISTRY["temperature"].unit,
                threshold="<= 0 °C (32 °F)",
                source=FREEZE_SOURCE,
    ...
    if pressure_change <= -6.0:
        alerts.append(
            build_alert(
                ...
                sensor="pressure",
                severity="warning",
                title="Descenso muy rápido de presión",
                message="La presión cayó más de 6.0 hPa en 3 horas, lo que sugiere un cambio meteorológico brusco.",
                recommendation="Seguir la evolución del sistema y contrastar con viento y lluvia.",
        ...
        alerts.append(
            build_alert(
                ...
                sensor="pressure",
                severity="advisory",
                title="Descenso rápido de presión",
                message="La presión cayó entre 3.6 y 6.0 hPa en 3 horas, rango asociado a una variación relevante.",
                recommendation="Observar si el descenso continúa y revisar el comportamiento del resto de sensores.",
                current_value=pressure_change,
                unit="hPa/3h",
    ...
    if value >= 60:
        alerts.append(
            build_alert(
                ...
                recommendation="Revisar soporte mecánico del sistema y continuar el monitoreo de ráfagas.",
                current_value=value,
    ...
    if record.rain_digital == "Lluvia":
        ...
        alerts.append(
            build_alert(
                ...
                recommendation="Revisar la evolución de lluvia analógica y velocidad del viento para seguir el evento.",
    ...
    if record.rain_analog is not None and record.rain_analog <= 900:
        alerts.append(
            build_alert(
                ...
                threshold="Lluvia analógica <= 900",
                source=RAIN_SOURCE,
                source_url=RAIN_SOURCE_URL,
                triggered_at=record.timestamp,
            ),
...
def evaluate_station_alerts(active: bool, active_label: str, record: SensorData | None) -> list[AlertDefinition]:
    ...
    return [
        build_alert(
            ...
            recommendation="Revisar conectividad, energía, API key y estado del microcontrolador o gateway.",
        ...
        if snapshot.station_active and not snapshot.current_packet_has_value and snapshot.last_packet_at is not None:
            alerts.append(
                build_alert(
                    ...
                    recommendation="Revisar cableado, lectura ADC/I2C y construcción del payload enviado por el microcontrolador.",
                    current_value=None,
        ...
        if snapshot.flatline_detected and snapshot.last_valid_at is not None:
            alerts.append(
                build_alert(
                    ...
                    message=f"{definition.label} repitió prácticamente el mismo valor durante {snapshot.flatline_length} paquetes seguidos.",
            ...
            if missing_ratio >= 0.4 and snapshot.last_packet_at is not None:
                alerts.append(
                    build_alert(
                        ...
                        sensor=metric_key,
                        severity="warning",
                        title=f"{definition.label} con pérdida alta de datos",
                        message=f"En las últimas 24 horas falta {round(missing_ratio * 100, 1)}% de las lecturas esperadas para {definition.label.lower()}.",
                        recommendation="Revisar estabilidad del sensor, del bus de comunicación y del mapeo del JSON enviado al backend.",
                        current_value=round(missing_ratio * 100, 1),
                        unit="%",
                        threshold="Pérdida >= 40% en el rango de 24h",
                        source=STATION_SOURCE,
                        source_url=STATION_SOURCE_URL,
                        triggered_at=snapshot.last_packet_at,
                    ),
...
def get_alerts_response(db: Session, station_id: str | None = None) -> AlertsResponse:
    ...
    if latest_record is None:
        waiting_alert = build_alert(
            ...
            recommendation="Verificar encendido, conectividad WiFi, API Key, endpoint y formato del JSON de envío.",
            current_value=None,
            unit=None,
            threshold="Sin telemetría inicial",
            source=STATION_SOURCE,
            source_url=STATION_SOURCE_URL,
