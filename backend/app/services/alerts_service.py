    ...
    if value >= 42:
        ...
        alerts.append(
            build_alert(
                ...
                message="La temperatura ambiental supera 38 °C y conviene revisar el comportamiento termico de la estacion.",
    ...
    if value <= -2.2:
        alerts.append(
            build_alert(
                ...
                message="La temperatura esta por debajo de 28 °F, rango considerado hard freeze por el NWS.",
        ...
        alerts.append(
            build_alert(
                ...
                message="La temperatura esta en o por debajo de 32 °F, rango asociado a condiciones de freeze warning.",
    ...
    if value >= 60:
        alerts.append(
            build_alert(
                ...
                message="La velocidad del viento supera 60 km/h y puede afectar la estabilidad de la estacion o su entorno.",
        ...
        alerts.append(
            build_alert(
                ...
                recommendation="Contrastar con presion, lluvia y comportamiento mecanico de la estacion.",
                current_value=value,
                unit=METRIC_REGISTRY["wind_speed"].unit,
    ...
    if record.rain_analog is not None and record.rain_analog <= 900:
        alerts.append(
            build_alert(
                ...
                message="La lectura analogica del MH-RD esta en un rango muy bajo, compatible con humedad intensa o lluvia sostenida.",
...
def evaluate_station_alerts(active: bool, active_label: str, record: SensorData | None) -> list[AlertDefinition]:
    ...
    return [
        build_alert(
            ...
            sensor="station",
            severity="warning",
            title="Estacion sin reporte reciente",
            message=f"La estacion figura como {active_label.lower()} y no ha enviado datos dentro de la ventana operativa configurada.",
        ...
        if snapshot.station_active and not snapshot.current_packet_has_value and snapshot.last_packet_at is not None:
            alerts.append(
                build_alert(
                    ...
                    message=f"La estacion sigue reportando, pero el ultimo paquete llego sin valor para {definition.label.lower()}.",
                    recommendation="Revisar cableado, lectura ADC/I2C y construccion del payload enviado por el microcontrolador.",
                    current_value=None,
                    unit=definition.unit,
                    threshold="Campo ausente en el ultimo paquete",
                    source=STATION_SOURCE,
                    source_url=STATION_SOURCE_URL,
        ...
        if snapshot.flatline_detected and snapshot.last_valid_at is not None:
            alerts.append(
                build_alert(
                    ...
                    recommendation="Validar si el sensor sigue midiendo o si el canal quedo congelado en el ultimo valor.",
    ...
    if latest_record is not None and latest_record.rain_digital is None:
        alerts.append(
            build_alert(
                ...
                message="El ultimo paquete no trajo el estado digital del sensor de lluvia.",
                recommendation="Revisar el pin digital del MH-RD y el armado del payload enviado por el ESP32.",
                current_value=None,
                unit=None,
                threshold="Campo rain_digital ausente en el ultimo paquete",
                source=STATION_SOURCE,
                source_url=STATION_SOURCE_URL,
...
def get_alerts_response(db: Session, station_id: str | None = None) -> AlertsResponse:
    ...
    if latest_record is None:
        waiting_alert = build_alert(
            ...
            sensor="station",
            severity="critical",
            title="No hay conexion con la estacion",
            message="La plataforma aun no ha recibido ningun paquete desde Arduino, ESP32 o el gateway intermediario.",
